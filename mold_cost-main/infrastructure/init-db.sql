...
CREATE INDEX idx_features_nc_time_cost ON features USING GIN(nc_time_cost);
...
COMMENT ON COLUMN features.nc_time_cost IS 'NC时间详细数据，JSON格式，格式: {"nc_details": [{"code": "L", "value": "5"}, {"code": "ZXZ", "value": "5"}, {"code": "开粗", "value": "5"}, {"code": "精铣", "value": "5"}]}';
COMMENT ON COLUMN features.is_complete IS '特征数据是否完整';
COMMENT ON COLUMN features.missing_params IS '缺失的参数列表';
COMMENT ON COLUMN features.extended_features IS '扩展特征，JSON格式';
