...
| R6 | P1 | CAD / feature 算法本体迁移 | 收尾中 | feature 分析、batch 编排、持久化、红面查表与红面写回入口已迁出，CAD split runtime、子图识别/导出编排、输入源解析、输入准备、子图编号编排、板料线后处理、`.x_t` 导出准备、上传与结果持久化边界已立，CAD 主流程仍在 legacy |
