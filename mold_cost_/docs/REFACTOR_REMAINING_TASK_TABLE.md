...
| R6 | P1 | CAD / feature 算法本体迁移 | 收尾中 | feature 分析、batch 编排、持久化、红面查表与红面写回入口已迁出，CAD split runtime、子图识别/导出编排、输入源解析、输入准备、子图编号编排、板料线后处理、`.x_t` 导出准备、PRT 组件匹配与 Parasolid 导出、上传与结果持久化边界已立，CAD 主流程仍在 legacy |
...
| R6 | CAD/feature 主链不再依赖 `scripts/*` | `feature_analysis_runtime.py`、`feature_batch_runtime.py`、`feature_persistence_runtime.py`、`slider_red_face_lookup_runtime.py`、`slider_red_face_update_runtime.py`、`cad_split_runtime.py`、`cad_analysis_runtime.py`、`cad_source_runtime.py`、`cad_prepare_runtime.py`、`cad_region_runtime.py`、`cad_material_line_runtime.py`、`cad_upload_runtime.py`、`cad_split_persistence_runtime.py`、`cad_xt_export_runtime.py` 已落地，且 `.x_t` 的 PRT 组件匹配与 Parasolid 导出实现已迁入 `src` | `chaitu_process` 与 `analysis_system` / `analyzer` / `MaterialLineIntegrator` 实现本体仍在 legacy | 优先继续拆 CAD split analyzer 实现本体与 material line 核心实现，再评估主流程 glue 继续切分 | `domain.cad` 与 `domain.features` 主链不再依赖 legacy gateway 指向脚本 |
