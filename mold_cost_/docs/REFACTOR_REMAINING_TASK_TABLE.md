...
| R4 Pricing 主链迁移收尾 | P1 | 完成 search、process matcher 与 calculator 的迁移 | 桥接中 | 第三批 search、`process_rule_matcher` 与五波 calculator `price_material / price_wire_total / price_total / price_weight / price_heat / price_nc_total / price_nc_base / price_tooth_hole / price_water_mill_total / price_add_auto_material / price_wire_base / price_wire_special / price_wire_standard / price_nc_time / judgment / price_water_mill_bevel_cost / price_water_mill_chamfer_cost / price_water_mill_component` 已迁完，`pricing_service.update_job_total_cost` 也已去 `shared.database` 依赖，但 `pricing_service.calculate` 仍桥接 `PricingAgent`，水磨细分项仍有脚本透传 | 继续替换 `price_water_mill_hanging_table`、`price_water_mill_high_cost`、`price_water_mill_long_strip` 等水磨尾项，随后缩小 `pricing_service.calculate` 对 agent 的依赖 | `domain/pricing` 主链不再直接 import `scripts.calculate/*` 或旧 pricing agent |
