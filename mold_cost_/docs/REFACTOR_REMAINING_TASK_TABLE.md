...
| R3 Review 数据访问去 legacy 化 | P0 | 把 review 数据加载、修改应用、状态管理完整收口到新层 | 桥接中 | repository 已收口，但 `review_change_applier` 仍调用 `agents.action_handlers`、`agents.confirm_handler`；默认 state/session 仍是 Redis 适配器 | 提取 review handler port，继续拆掉 legacy handler 调用 | `review_graph` 默认装配只依赖 `src/mold_cost` 内部对象 |
| R4 Pricing 主链迁移收尾 | P1 | 完成 search、process matcher 与 calculator 的迁移 | 桥接中 | 第三批 search、`process_rule_matcher` 与 calculator 首批 `price_material / price_wire_total / price_total` 已迁完，但 `pricing_service.calculate` 仍桥接 `PricingAgent`，其余 calculator 仍大量脚本透传 | 继续替换 `price_heat`、`price_weight`、`price_nc_total` 等剩余 calculator，并继续缩小 `pricing_service` 对 agent 的依赖 | `domain/pricing` 主链不再直接 import `scripts.calculate/*` 或旧 pricing agent |
