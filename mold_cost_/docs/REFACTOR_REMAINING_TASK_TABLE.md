...
| R4 Pricing 主链迁移收尾 | P1 | 完成 search、process matcher 与 calculator 的迁移 | 桥接中 | `domain.pricing.search`、`process_rule_matcher` 与 `domain.pricing.calculators` 已全部去掉 `scripts.calculate/*` 直连，`pricing_service.update_job_total_cost` 也已去 `shared.database` 依赖；当前残留桥接已收敛到 `pricing_service.calculate -> PricingAgent`、`agents/pricing_agent_local.py` 以及 MCP 兼容路由 | 优先下沉 `pricing_service.calculate`，再缩减 `agents/pricing_agent_local.py` 与 `mcp_services/cad_price_search_mcp/server.py` 的兼容编排壳层 | `domain/pricing` 主链与外部入口不再依赖旧 pricing agent 作为运行时桥接 |
