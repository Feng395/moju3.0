"""Material pricing calculator implementation."""
...
MCP_TOOL_META = {
    ...
    "description": "Calculate material cost from base itemcode, material price and density data.",
    "inputSchema": {
        "type": "object",
        "properties": {
            "search_data": {
...
async def _calculate_part_cost(
    ...
    logger.info("Calculating cost for part: %s (%s), material: %s", part_name, subgraph_id, material)
    ...
    material_cost = (weight * Decimal(str(unit_price))).quantize(Decimal("0.01"), ROUND_HALF_UP)
    ...
    logger.info(
        "[%s] %s: material=%s, weight=%s, unit_price=%s, material_cost=%s",
        subgraph_id,
        part_name,
        original_material,
        weight,
...
async def batch_upsert_with_steps(
    ...
    if field_name != "material_cost":
        raise ValueError(f"Unsupported field_name for material calculator: {field_name}")
    ...
    if failures:
        raise failures[0]
    logger.info("Successfully batch updated %s/%s records", len(updates), len(updates))
...
async def _upsert_single_record(
    ...
    try:
        if field_name == "material_cost":
            ...
            await db.execute(_MATERIAL_UPSERT_SQL, job_id, subgraph_id, field_value, category, steps_json)
            return True
        raise ValueError(f"Unsupported field_name: {field_name}")
    except Exception as exc:  # pragma: no cover - defensive logging
        logger.error("Failed to upsert record for %s: %s", subgraph_id, exc)
        raise
...
async def calculate(
    ...
    """Calculate material cost for each part in base_itemcode search results."""
    ...
    logger.info(
        "Calculating material cost for job_id: %s, parts count: %s",
        job_id,
