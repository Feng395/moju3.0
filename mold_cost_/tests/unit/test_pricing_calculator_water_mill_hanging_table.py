...
MODULE_PATH = "mold_cost.domain.pricing.calculators.price_water_mill_hanging_table"
...
def test_water_mill_hanging_table_calculator_small_and_large_paths(monkeypatch):
    module = _load_module()
                    ...
                    "parts": [
                        {
                            ...
                            "water_mill": {"water_mill_details": [{"hanging_table": 3}]},
                        },
                        {
                            ...
                            "water_mill": {"water_mill_details": [{"hanging_table": 2}]},
                        },
                    ],
                },
                "water_mill": {
                    "s_water_mill_prices": [
                        {"sub_category": "hanging_table", "price": 7.5, "unit": "piece"}
                    ]
                },
            }
        )
    ...
    assert category == "water_mill_hanging_table"
    assert field_name == "hanging_table_cost"
    assert len(updates) == 2
    assert updates[0]["job_id"] == "job-hanging-table-1"
    assert updates[0]["subgraph_id"] == "sg-small"
    assert updates[0]["value"] == pytest.approx(22.5)
    assert len(updates[0]["steps"]) == 4
    assert updates[1]["job_id"] == "job-hanging-table-1"
    assert updates[1]["subgraph_id"] == "sg-large"
    assert updates[1]["value"] == pytest.approx(0.0)
    assert len(updates[1]["steps"]) == 3


def test_water_mill_hanging_table_calculator_sync_wrapper_and_missing_data(monkeypatch):
    module = _load_module()
    captured = _patch_batch_upsert(monkeypatch, module)
    ...
    assert result["job_id"] == "job-hanging-table-2"
    assert len(result["results"]) == 1
    assert result["results"][0]["hanging_table_cost"] == pytest.approx(0.0)
    assert "no water_mill_details" in result["results"][0]["note"]
    assert captured == []


def test_water_mill_hanging_table_module_no_longer_imports_legacy_script():
    from mold_cost.domain.pricing.calculators import price_water_mill_hanging_table as module

    source = Path(module.__file__).read_text(encoding="utf-8-sig")
    assert "scripts.calculate.price_water_mill_hanging_table" not in source
