...
class DWGConverter:
    """DWG <-> DXF converter backed by ODA File Converter."""

    def __init__(self, oda_converter_path: str = None):
        self.oda_converter_path = oda_converter_path

    def _resolve_oda_converter_path(self):
        """Normalize the configured converter path and try safe local fallbacks."""
        if not self.oda_converter_path:
            return None

        raw_path = str(self.oda_converter_path).strip().strip('"').strip("'")
        if not raw_path:
            return None
        ...
        if "\\tool\\" in normalized:
            candidates.append(normalized.replace("\\tool\\", "\\tools\\"))
        if "\\tools\\" in normalized:
            candidates.append(normalized.replace("\\tools\\", "\\tool\\"))
    ...
    def _convert(self, input_file, output_file, output_format, output_version="ACAD2004"):
        """Convert a file with ODAFileConverter.exe."""
        if not os.path.exists(input_file):
            logger.debug(f"错误：找不到输入文件 {input_file}")
            return None

        resolved_oda_path = self._resolve_oda_converter_path()
        if not resolved_oda_path:
