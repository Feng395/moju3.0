...
class OptimizedTripleConditionProcessor:
            ...
            for layer, lines in lines_by_layer.items():
                if len(lines) >= 4:  # 至少4条线
                    ...
                    if len(horizontal) >= 2 and len(vertical) >= 2:
                        ...
                        # 过滤不合理的尺寸（放宽限制以支持大尺寸零件）
                        if 10 < width < 5000 and 10 < height < 5000 and 100 < area < 25000000:
    ...
    def _find_cross_layer_rectangles(self, center_position: tuple, search_radius: float) -> List[Dict]:
        """
        从跨图层的LINE实体中查找矩形视图
        用于识别由不同图层的LINE组成的视图（例如：U2图层的垂直线 + NOTE图层的水平线）
        
        策略：
        1. 收集所有附近的水平LINE和垂直LINE（不区分图层）
        2. 尝试组合成矩形
        3. 验证矩形的合理性
        
        改进：尝试所有可能的垂直线对和水平线对组合,不要求相邻
        """
        rectangles = []
        ...
        try:
            ...
            for v_line in vertical:
                x_pos = (v_line.dxf.start[0] + v_line.dxf.end[0]) / 2
                found_group = False
                for group in v_groups:
                    if abs(group['x'] - x_pos) < 5.0:
                        group['lines'].append(v_line)
                        # 更新Y范围
                        y_coords = [v_line.dxf.start[1], v_line.dxf.end[1]]
                        group['y_min'] = min(group.get('y_min', float('inf')), min(y_coords))
                        group['y_max'] = max(group.get('y_max', float('-inf')), max(y_coords))
                        found_group = True
                        break
                if not found_group:
                    y_coords = [v_line.dxf.start[1], v_line.dxf.end[1]]
            ...
            for h_line in horizontal:
                y_pos = (h_line.dxf.start[1] + h_line.dxf.end[1]) / 2
                found_group = False
                for group in h_groups:
                    if abs(group['y'] - y_pos) < 5.0:
                        group['lines'].append(h_line)
                        # 更新X范围
                        x_coords = [h_line.dxf.start[0], h_line.dxf.end[0]]
                        group['x_min'] = min(group.get('x_min', float('inf')), min(x_coords))
                        group['x_max'] = max(group.get('x_max', float('-inf')), max(x_coords))
                        found_group = True
                        break
                if not found_group:
                    x_coords = [h_line.dxf.start[0], h_line.dxf.end[0]]
            ...
            # ✅ 优化：限制组合数量，避免四重循环导致性能问题
            if len(v_groups) >= 2 and len(h_groups) >= 2:
                # 按X坐标排序垂直线组
                v_groups.sort(key=lambda g: g['x'])
                # 按Y坐标排序水平线组
                h_groups.sort(key=lambda g: g['y'])
                
                # ✅ 限制：只尝试相邻或距离较近的组对，避免组合爆炸
                max_v_pairs = 20  # 最多20对垂直线组
                max_h_pairs = 20  # 最多20对水平线组
                
                v_pairs = []
                # 优先选择相邻的垂直线组对
                for i in range(len(v_groups)):
                    for j in range(i + 1, min(i + 5, len(v_groups))):  # 只看后面4个
                        v_left = v_groups[i]
                        v_right = v_groups[j]
                        width = v_right['x'] - v_left['x']
                        if 10 < width < 5000:  # ✅ 放宽宽度限制到5000mm
                            v_pairs.append((v_left, v_right, width))
                            if len(v_pairs) >= max_v_pairs:
                                break
                    if len(v_pairs) >= max_v_pairs:
                        break
                ...
                h_pairs = []
                # 优先选择相邻的水平线组对
                for m in range(len(h_groups)):
                    for n in range(m + 1, min(m + 5, len(h_groups))):  # 只看后面4个
                        h_bottom = h_groups[m]
                        h_top = h_groups[n]
                        height = h_top['y'] - h_bottom['y']
                        if 10 < height < 5000:  # ✅ 放宽高度限制到5000mm
                            h_pairs.append((h_bottom, h_top, height))
                            if len(h_pairs) >= max_h_pairs:
                                break
                    if len(h_pairs) >= max_h_pairs:
                        break
                
                # 组合垂直线对和水平线对
                for v_left, v_right, width in v_pairs:
                    for h_bottom, h_top, height in h_pairs:
                        # 验证是否形成合理的矩形
                        area = width * height
                        if not (100 < area < 25000000):  # ✅ 放宽面积限制到25平方米
                            continue
                        
                        # 验证垂直线和水平线是否有重叠（形成闭合矩形）
                        # 垂直线的Y范围应该覆盖水平线的Y坐标
                        if not (v_left['y_min'] <= h_bottom['y'] <= v_left['y_max'] and
                               v_left['y_min'] <= h_top['y'] <= v_left['y_max']):
                            continue
                        if not (v_right['y_min'] <= h_bottom['y'] <= v_right['y_max'] and
                               v_right['y_min'] <= h_top['y'] <= v_right['y_max']):
                            continue
                        
                        # 水平线的X范围应该覆盖垂直线的X坐标
                        if not (h_bottom['x_min'] <= v_left['x'] <= h_bottom['x_max'] and
                               h_bottom['x_min'] <= v_right['x'] <= h_bottom['x_max']):
                            continue
                        if not (h_top['x_min'] <= v_left['x'] <= h_top['x_max'] and
                               h_top['x_min'] <= v_right['x'] <= h_top['x_max']):
                            continue
                        
                        # 构建bbox
                        bbox = (v_left['x'], h_bottom['y'], v_right['x'], h_top['y'])
                        center_x = (bbox[0] + bbox[2]) / 2
                        center_y = (bbox[1] + bbox[3]) / 2
                        ...
                        # 收集涉及的图层
                        layers = set()
                        for line in v_left['lines'] + v_right['lines'] + h_bottom['lines'] + h_top['lines']:
                        ...
                        rectangles.append({
                            ...
                            'height': height,
                            'area': area,
                            'type': 'cross_layer_rectangle',
                            'layers': list(layers)
                        })
                
                # ✅ 只在找到矩形时才打印，避免日志刷屏
                if rectangles:
                    print(f"  >> 从跨图层LINE识别到 {len(rectangles)} 个矩形")
        
        except Exception as e:
            print(f"⚠️ 从跨图层LINE识别矩形失败: {e}")
            import traceback
            traceback.print_exc()
            ...
            for layer_name in target_layers:
                ...
                if entities:
                    ...
                    if xs and ys:
                        ...
                        # 跳过明显过大的边界框（可能是整个图层）
                        if distance <= search_radius and 10 < width < 5000 and 10 < height < 5000 and 100 < area < 25000000:
                            views.append({
                                'center': (center_x, center_y),
                                'bbox': bbox,
                                'distance': distance,
    ...
    def _find_all_matching_views_globally(self, l: float, w: float, t: float) -> Dict[str, Dict]:
        ...
        for entity in self.msp.query('LWPOLYLINE'):
            ...
            # 过滤不合理的尺寸（放宽上限以支持大尺寸零件）
            if area < 500 or area > 25000000:
                continue
    ...
    def _identify_views_by_dimensions_and_position(self, candidate_views: List[Dict], l: float, w: float, t: float) -> Dict[str, Dict]:
        ...
        tolerance = 10.0  # 尺寸容差10mm（放宽以匹配更多实际图纸）
        identified_views = {}
        ...
        for view in candidate_views:
            ...
            if (abs(w_view - l) < tolerance and abs(h_view - w) < tolerance) or \
               (abs(w_view - w) < tolerance and abs(h_view - l) < tolerance):
                # 计算误差
                error1 = abs(w_view - l) + abs(h_view - w)
                error2 = abs(w_view - w) + abs(h_view - l)
                error = min(error1, error2)
                view_candidates['主视图'].append((view, error))
            
            # 俯视图 (L×T)
            if (abs(w_view - l) < tolerance and abs(h_view - t) < tolerance) or \
                 (abs(w_view - t) < tolerance and abs(h_view - l) < tolerance):
                error1 = abs(w_view - l) + abs(h_view - t)
                error2 = abs(w_view - t) + abs(h_view - l)
                error = min(error1, error2)
                view_candidates['俯视图'].append((view, error))
            
            # 侧视图 (T×W)
            if (abs(w_view - t) < tolerance and abs(h_view - w) < tolerance) or \
                 (abs(w_view - w) < tolerance and abs(h_view - t) < tolerance):
                error1 = abs(w_view - t) + abs(h_view - w)
                error2 = abs(w_view - w) + abs(h_view - t)
                error = min(error1, error2)
                view_candidates['侧视图'].append((view, error))
        
        # 为每种视图类型选择误差最小的候选
        for view_type, candidates in view_candidates.items():
            if candidates:
                # 按误差排序,选择最小的
                candidates.sort(key=lambda x: x[1])
                best_view, best_error = candidates[0]
                identified_views[view_type] = best_view
                print(f"     ✅ {view_type}: {best_view['width']:.1f}x{best_view['height']:.1f} @ ({best_view['center'][0]:.1f}, {best_view['center'][1]:.1f}), 误差={best_error:.1f}mm")
    ...
    def _collect_all_views(self, text_position: tuple, search_radius: float) -> List[Dict]:
            ...
            # 2.1 跨图层LINE组成的矩形（新增：解决主视图跨图层问题）
            # ✅ 限制：如果已经有足够多的候选视图，跳过跨图层识别（性能优化）
            if len(candidate_views) < 100:
                cross_layer_rectangles = self._find_cross_layer_rectangles(text_position, search_radius)
                candidate_views.extend(cross_layer_rectangles)
            else:
                print(f"  >> 跳过跨图层识别（已有{len(candidate_views)}个候选视图）")
            ...
            # 3. 按图层分组的视图
            # ✅ 限制：如果已经有足够多的候选视图，跳过图层视图识别
            if len(candidate_views) < 150:
                ...
                print(f"  >> 跳过图层视图识别（已有{len(candidate_views)}个候选视图）")
            
            # ✅ 早期过滤：如果候选视图过多，优先保留大尺寸和距离近的视图
            if len(candidate_views) > 200:
                print(f"  >> 候选视图过多({len(candidate_views)}个)，优先保留大尺寸视图")
                # 综合评分：面积越大越好，距离越近越好
                for view in candidate_views:
                    # 归一化距离（0-1，越小越好）
                    norm_distance = view['distance'] / search_radius if search_radius > 0 else 0
                    # 归一化面积（0-1，越大越好）
                    norm_area = min(view['area'] / 1000000, 1.0)  # 1平方米为参考
                    # 综合评分：面积权重70%，距离权重30%
                    view['priority_score'] = norm_area * 0.7 - norm_distance * 0.3
                
                candidate_views.sort(key=lambda v: v['priority_score'], reverse=True)
                candidate_views = candidate_views[:200]

            # ✅ 优化去重：使用字典加速查找，避免O(n²)复杂度
            unique_views = []
            tolerance_pos = 10.0  # 位置容差10mm
            tolerance_size = 2.0  # 尺寸容差2mm
            
            # 使用网格索引加速空间查询
            grid_size = tolerance_pos * 2
            grid_dict = {}  # key: (grid_x, grid_y), value: list of views
            ...
            for view in candidate_views:
                ...
                for dx in [-1, 0, 1]:
                    for dy in [-1, 0, 1]:
                        ...
                        for existing in grid_dict[grid_key]:
                            # 检查位置和尺寸是否相同
                            pos_diff = ((cx - existing['center'][0])**2 + 
                                       (cy - existing['center'][1])**2)**0.5
                ...
                if not is_duplicate:
                    ...
                    if grid_key not in grid_dict:
                        grid_dict[grid_key] = []
                    grid_dict[grid_key].append(view)
            ...
            unique_views.sort(key=lambda v: v['priority_score'], reverse=True)
            
            if len(unique_views) < len(candidate_views):
                print(f"  >> 去重完成：{len(candidate_views)}个视图 → {len(unique_views)}个唯一视图")
            else:
                print(f"  >> 收集完成：共{len(unique_views)}个视图")
    ...
    def _infer_lwt_from_views_by_geometry(self, views: List[Dict]) -> Optional[Dict[str, float]]:
        ...
        # 过滤有效视图（排除明显错误的边界框）
        # 放宽面积上限，支持大尺寸零件
        valid_views = [v for v in views if 100 < v['area'] < 25000000]
        if not valid_views:
            print(f"  >> ❌ 没有有效视图")
            return None
