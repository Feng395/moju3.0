...
def process_single_part(dxf_file_path: str, output_dir: str, fail_dir: str, log_file_dir: str, json_0_0_dir: str,
    ...
    if output_file_path is None:
        output_file_path = os.path.join(output_dir, file_name)  # 使用原始文件名
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        print(f"⚠️ 文件被占用，等待1秒后重试... (尝试 {attempt + 1}/{max_retries})")
                        import time
                        time.sleep(1)
                    else:
                        # 最后一次尝试：使用备用文件名
                        ...
                        backup_path = output_file_path.replace('.dxf', f'_{timestamp}.dxf')
                        try:
                            doc.saveas(backup_path)
                            print(f"⚠️ 原文件被占用，已保存到备用路径: {backup_path}")
                            print(f"💡 提示：请关闭占用文件的程序（如CAD软件），然后重命名备用文件")
                            return True
                        except Exception as e2:
                            print(f"❌ 保存失败: {pe}")
                            print(f"❌ 备用保存也失败: {e2}")
                            print(f"💡 请检查：")
                            print(f"   1. 文件是否被CAD软件或其他程序打开")
                            print(f"   2. 输出目录是否有写入权限")
                            print(f"   3. 磁盘空间是否充足")
                            return False
            return False
        else:
...
if __name__ == "__main__":
    ...
    import sys
    if len(sys.argv) > 1:
        if sys.argv[1] == "--test-duplicate":
            dxf_file = r"D:\my_project\cadagent\sheet_line\output\ceshitua1_with_all_material_lines.dxf"
        ...
        # 默认文件（使用相对路径，自动适应当前目录）
        current_dir = os.path.dirname(os.path.abspath(__file__))
        dxf_file = os.path.join(current_dir, "ceshitua3.dxf")
        
        # 如果当前目录没有，尝试查找其他DXF文件
        if not os.path.exists(dxf_file):
            dxf_files = [f for f in os.listdir(current_dir) if f.endswith('.dxf')]
            if dxf_files:
                dxf_file = os.path.join(current_dir, dxf_files[0])
                print(f"📄 使用找到的文件: {dxf_file}")
            else:
                print(f"❌ 错误：当前目录没有找到DXF文件")
                print(f"   目录: {current_dir}")
                sys.exit(1)
        else:
            print(f"📄 使用默认文件: {dxf_file}")
