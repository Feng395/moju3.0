...
4. 判断大小水磨：
   - 小水磨：计算 small_grinding_cost = (chamfer_cost+bevel_cost+oil_tank_cost)*quantity*50 + thread_ends_cost+hanging_table_cost+high_cost
...
MCP_TOOL_META = {
    ...
    "description": "计算水磨总价和时间：根据大小水磨类型计算总费用和加工时间，更新到 subgraphs 表",
    "inputSchema": {
        "type": "object",
        "properties": {
            "search_data": {
...
async def calculate(
    ...
    if db_updates:
        # 分别更新小水磨和大水磨的 calculation_steps
...
def _build_price_map(water_mill_data: Dict[str, Any]) -> Dict[str, float]:
    ...
    Returns:
        {
            "small_hourly_rate": 50,  # 小水磨时薪（元/小时）
    ...
    # 处理小水磨价格（从 s_water_mill_prices 中查找）
...
async def _calculate_part_total(
    ...
    calculation_steps.append({
        ...
        "mill_type": mill_type,
        "description": "小水磨" if mill_type == "s_water_mill" else "大水磨"
    })
...
def _calculate_small_grinding_cost(costs: Dict, quantity: int, hourly_rate: float) -> tuple[float, float, List[Dict]]:
    ...
    公式：(chamfer_cost+bevel_cost+oil_tank_cost)*quantity*50 + thread_ends_cost+hanging_table_cost+high_cost
    
    单位说明：
    - thread_ends_cost: 元
    - hanging_table_cost: 元
    - high_cost: 元
    ...
    # 计算固定费用部分：thread_ends_cost + hanging_table_cost + high_cost
    fixed_cost_total = thread_ends_cost + hanging_table_cost + high_cost
    
    calculation_steps.append({
        "step": "计算固定费用",
        "formula": f"{thread_ends_cost} + {hanging_table_cost} + {high_cost}",
