...
def apply_red_face_lookup(
    ...
    """
    主入口：用 part_code 查 MinIO 上的 feature_database.json，
    将红色面数据补充到 wire_cut_details 中 instruction 含"滑"的条目。

    规则：
    - 只更新 area_num / total_length / single_length / matched_count / expected_count
    - code 强制设为 '滑块'，view 强制设为 'front_view'
    - 其余字段（slider_angle / matched_line_ids 等）保留原值
    - 若 wire_cut_details 中没有含"滑"的条目但数据库有记录，则追加新条目

    Args:
        part_code:        零件编号，如 "DIE-06"
        wire_cut_details: 当前的线割详情列表
        minio_client:     MinIO 客户端实例（None 时跳过查表）
        minio_db_path:    MinIO 中 JSON 文件路径，None 时用环境变量或默认值

    Returns:
        更新后的 wire_cut_details（原列表不修改，返回新列表）
    """
    ...
    single_len   = db_slider.get('single_length', 0.0)
    ...
    for d in wire_cut_details:
        if '滑' in d.get('instruction', ''):
            new_d = dict(d)          # 浅拷贝，保留 slider_angle 等字段
            new_d['code']           = '滑块'
            new_d['view']           = 'front_view'
            new_d['area_num']       = area_num
            new_d['total_length']   = total_length
            new_d['single_length']  = single_len
            new_d['matched_count']  = area_num
            new_d['expected_count'] = area_num
            updated.append(new_d)
            patched = True
            logger.info(
                f"[{part_code}] 查表补充滑块红色面: "
    ...
    if not patched:
        updated.append({
            ...
            'single_length':     single_len,
            'expected_count':    area_num,
            'matched_line_ids':  [],
            'overlapping_length': 0.0,
        })
        logger.info(
            f"[{part_code}] 查表追加滑块红色面条目: "
            f"area_num={area_num}, total_length={total_length}mm²"
        )
