    ...
    try:
        ...
        for row in rows:
            ...
            detail = {
                ...
                "nc_roughing_cost": float(row["nc_roughing_cost"]) if row["nc_roughing_cost"] is not None else 0.0,
                "nc_milling_cost": float(row["nc_milling_cost"]) if row["nc_milling_cost"] is not None else 0.0,
                "nc_drilling_cost": float(row["nc_drilling_cost"]) if row["nc_drilling_cost"] is not None else 0.0,
                "nc_base_roughing_cost": float(row["nc_base_roughing_cost"]) if row["nc_base_roughing_cost"] is not None else 0.0,
                "nc_base_milling_cost": float(row["nc_base_milling_cost"]) if row["nc_base_milling_cost"] is not None else 0.0,
                "nc_base_drilling_cost": float(row["nc_base_drilling_cost"]) if row["nc_base_drilling_cost"] is not None else 0.0,
...
# logging.basicConfig(...)
    ...
    for detail in results['cost_details']:
        ...
        print(f"  nc_roughing_cost: {detail['nc_roughing_cost']} 元")
        print(f"  nc_milling_cost: {detail['nc_milling_cost']} 元")
        print(f"  nc_drilling_cost: {detail['nc_drilling_cost']} 元")
        print(f"  nc_base_roughing_cost: {detail['nc_base_roughing_cost']} 元")
        print(f"  nc_base_milling_cost: {detail['nc_base_milling_cost']} 元")
        print(f"  nc_base_drilling_cost: {detail['nc_base_drilling_cost']} 元")
