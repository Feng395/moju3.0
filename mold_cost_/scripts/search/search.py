...
MCP_TOOL_META = {
    ...
    "description": "检索 subgraphs 表的成本汇总：获取各项总价（需在 price_wire_total 和 price_water_mill_total 执行后调用）",
    "inputSchema": {
        "type": "object",
        "properties": {
            "job_id": {
    ...
    try:
        ...
        for row in rows:
            summary = {
                ...
                "drilling_cost": float(row["drilling_cost"]) if row["drilling_cost"] is not None else 0.0
            }
...
if __name__ == "__main__":
    ...
    for summary in results['cost_summary']:
        ...
        print(f"  nc_roughing_cost: {summary['nc_roughing_cost']} 元")
        print(f"  nc_milling_cost: {summary['nc_milling_cost']} 元")
        print(f"  drilling_cost: {summary['drilling_cost']} 元")
        print()
