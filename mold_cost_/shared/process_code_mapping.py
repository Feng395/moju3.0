...
PROCESS_DETAIL_MAPPING = {
    ...
    "慢丝": {
        "category": "wire",
        "sub_category": "slow_and_one",  # 默认慢丝割一修一
        "note": "慢丝割一修一"
    },
...
def resolve_process_code(chinese_text: str) -> Optional[Dict[str, Any]]:
    ...
    Args:
        chinese_text: 中文工艺名称（如"慢丝割一修一"、"线割割一修一"）
    ...
    for key, value in PROCESS_DETAIL_MAPPING.items():
        if key in chinese_text or chinese_text in key:
            logger.info(f"✅ 工艺代码模糊匹配: {chinese_text} → {key} → {value}")
            return value
...
def extract_process_from_text(text: str) -> Optional[Dict[str, Any]]:
    ...
    Args:
        text: 用户输入的自然语言（如"将这套的线割割一修一的单价改成0.0018"或"材料Cr12的价格改为5"）
    
    Returns:
        工艺/材质代码字典，如果未找到返回 None
    
    Examples:
        >>> extract_process_from_text("将这套的线割割一修一的单价改成0.0018")
        {'category': 'wire', 'sub_category': 'slow_and_one', 'note': '慢丝割一修一'}
