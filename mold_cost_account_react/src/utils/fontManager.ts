...
export async function preloadFont(fontName: string): Promise<boolean> {
  try {
    ...
    if (response.ok) {
      console.log(`✅ 字体预加载成功: ${fontName} -> ${fontUrl}`)
      return true
    } else {
      console.log(`⚠️ 字体文件不存在: ${fontName} -> ${fontUrl}`)
      return false
    }
  } catch (error) {
...
export async function preloadCommonFonts(): Promise<string[]> {
  ...
  console.log(`✅ 成功预加载 ${loadedFonts.length}/${COMMON_DXF_FONTS.length} 个字体`)
