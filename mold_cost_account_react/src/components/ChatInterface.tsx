...
const ChatInterface: React.FC = () => {
        ...
        try {
            ...
            if (requestJobId !== useAppStore.getState().currentJobId) {
                console.log('⚠️ 会话已切换，忽略旧会话的响应', {
                    requestJobId,
                    currentJobId: useAppStore.getState().currentJobId
                });
                setIsTyping(false);
                return;
            }
    ...
    }, [inputValue, isLoggedIn, currentJobId, setShowLoginModal, setMessages, addMessage, setIsTyping]);
        ...
        >
            ...
            >
                <Flex align="center" justify="space-between" style={{ width: '100%' }}>
                    ...
                    {/* 右侧：登录按钮（仅未登录时显示）或进度条控制按钮（登录后显示） */}
                    {!isLoggedIn ? (
                        <Button
                            type="primary"
                            icon={<LoginOutlined />}
                        ...
                        <Button
                            type="text"
                            icon={showProgressBar ? <EyeInvisibleOutlined /> : <EyeOutlined />}
