...
const Sidebar: React.FC = () => {
  ...
  const sidebarContent = (
    <div style={{ 
      ...
      {/* 头部固定区域 - Logo */}
      <div style={{ 
        padding: (isMobile ? false : sidebarCollapsed) ? '16px 10px' : '16px 20px',
        flexShrink: 0, // 固定不收缩
          ...
          {sidebarCollapsed ? (
            ...
            <div
              style={{
                ...
                background: isHoveringLogo ? token.colorFillTertiary : 'transparent',
                border: `1px solid ${isHoveringLogo ? token.colorBorderSecondary : 'transparent'}`,
              }}
      ...
      <div style={{
        ...
        <div style={{ padding: '0 10px', marginTop: 16 }}>
          ...
          >
            <Flex align="center" gap={8}>
              <span style={{ fontSize: 16, color: token.colorText }}>
                <PlusOutlined />
              </span>
            ...
            {menuItems.map(item => (
              <div
                ...
                style={{
                  ...
                  height: 40,
                  borderRadius: token.borderRadius,
                  background: currentView === item.key ? token.colorFillTertiary : 'transparent',
                  transition: 'all 0.2s',
                  border: `1px solid ${currentView === item.key ? token.colorBorderSecondary : 'transparent'}`,
                  cursor: 'pointer',
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: (isMobile ? false : sidebarCollapsed) ? 'center' : 'flex-start',
                        ...
                        <Flex justify="space-between" align="center">
                          <div
                            style={{
                              ...
                              whiteSpace: 'nowrap',
                              color: isSelected ? 'rgba(0, 0, 0, 1)' : 'rgba(0, 0, 0, 0.6)',
                              cursor: 'pointer',
                              flex: 1,
                              marginRight: 8,
                            }}
                              ...
                              try {
                                ...
                                await websocketService.connect(session.job_id, {
                                  ...
                                  onProgress: (jobId, data) => {
                                    ...
                                    console.log('📊 收到进度消息:', data, '消息jobId:', jobId, '当前jobId:', latestCurrentJobId)
                                    
                                    // 检查消息的 job_id 是否与当前会话的 job_id 一致
                                    if (jobId !== latestCurrentJobId) {
                                      console.log('⚠️ 忽略其他会话的进度消息:', {
                                        messageJobId: jobId,
                                        currentJobId: latestCurrentJobId
                                      })
                                      return
                                    }
                                    
                                    console.log('✅ job_id 匹配，处理进度消息')
                  ...
                  {sessions.length >= 5 && (
                    <div
                      ...
                      style={{
                        padding: '8px 10px',
                        borderRadius: token.borderRadius,
                        background: currentView === 'history' ? token.colorFillTertiary : 'transparent',
                        transition: 'all 0.2s',
                        border: `1px solid ${currentView === 'history' ? token.colorBorderSecondary : 'transparent'}`,
                        position: 'relative',
                        marginTop: 4,
                      }}
                      onMouseEnter={(e) => {
                      ...
                      <Flex justify="space-between" align="center">
                        <div
                          style={{
                            ...
                            textOverflow: 'ellipsis',
                            whiteSpace: 'nowrap',
                            color: currentView === 'history' ? 'rgba(0, 0, 0, 1)' : 'rgba(0, 0, 0, 0.6)',
                            cursor: 'pointer',
                            flex: 1,
                            marginRight: 8,
                          }}
